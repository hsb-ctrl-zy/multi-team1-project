...
# 전체 변경 핵심 요점
# 1. page_id 매핑 방식 변경 (복합 키 적용)
    # 기존: product_name 단일 키 매핑
    # 변경: (brand_name, product_name) 복합 키 매핑으로 동명이인/동일 상품명 매핑 오류 방지

# 2. 신규 이미지 DB 중복 검사 로직 추가
    # 기존: 데이터프레임 내부 중복만 제거 후 적재
    # 변경: DB의 image_data_table을 미리 조회하여 기존 (page_id, image_sequence) 중복 건 제외 후 INSERT

# 3. raw_data_table 중복 제거 조건 보완
    # 기존: drop_duplicates(subset=["product_name"])
    # 변경: drop_duplicates(subset=["brand_name", "product_name"])

# 4. 로그 출력 상세화
    # 이미지 적재 시 원본 수집 건수, page_id 매핑 성공 건수, 최종 DB 적재 건수를 세부 구분하여 출력하도록 수정을 거침
...
# 1. 파일별 전처리 헬퍼 함수
# -------------------------------------------------------------------
def process_product_csv(file_path: Path, tag_prefix: str = "") -> pd.DataFrame:
    """Product CSV 처리"""
    print(f"📦 [Product CSV] Reading: {file_path.name}")
    target_cols = ["cat", "CAT", "상품명"]
...
def process_text_csv(file_path: Path, tag_prefix: str = "") -> pd.DataFrame:
    """Text CSV 처리"""
    print(f"📝 [Text CSV] Reading: {file_path.name}")
    target_cols = ["상품명", "본문텍스트"]
...
def process_vl_csv(file_path: Path, tag_prefix: str = "") -> pd.DataFrame:
    """VL CSV 파일 처리"""
    print(f"🔍 [VL CSV] Reading: {file_path.name}")
    target_cols = ["상품명", "Text_contents"]
    ...
    print(f"   └ 완료 ({len(result_df):,}행 정제됨)")
    return result_df


def process_image_csv(file_path: Path, tag_prefix: str = "") -> pd.DataFrame:
    """Image CSV 파일 처리 (표준 컬럼으로 리네임 및 기본 정제)"""
...
def update_existing_raw_data(standalone_text_dfs: list, standalone_vl_dfs: list):
    """기존 DB 데이터에 text_contents 또는 image_text를 UPDATE"""
...
def insert_new_image_data(image_dfs: list):
    """신규 상품용 이미지 데이터 DB INSERT"""
    if not image_dfs:
        print("\n⚠️ [Image 적재 스킵] 처리할 신규 이미지 데이터가 없습니다.")
        return

    raw_image_df = pd.concat(image_dfs, ignore_index=True)
    if raw_image_df.empty:
        print("\n⚠️ [Image 적재 스킵] 이미지 데이터프레임이 비어 있습니다.")
        return

    total_raw_rows = len(raw_image_df)
    total_raw_products = raw_image_df["product_name"].nunique()

    print(f"\n🖼️ [신규 Image 적재 시작] 수집 완료: {total_raw_products:,}개 상품 / 총 {total_raw_rows:,}개 이미지 URL")

    # 1. DB에서 page_id 매핑용 데이터 조회
    ...
    # 3. page_id 매핑
    page_map = (
        product_id_df.drop_duplicates(subset=["brand_name", "product_name"], keep="last")
        .set_index(["brand_name", "product_name"])["page_id"]
        .to_dict()
    )

    img_df["page_id"] = img_df.apply(
        lambda r: page_map.get((r["brand_name"], r["product_name"])), axis=1
    )

    matched_count = img_df["page_id"].notna().sum()
    if matched_count == 0:
        print("   ❌ 브랜드와 상품명이 일치하는 raw_data_table 레코드를 찾지 못했습니다.")
        return

    img_db_df = img_df.dropna(subset=["page_id"]).copy()
    img_db_df["page_id"] = img_db_df["page_id"].astype(int)

    # Boolean 및 Alt 텍스트 기본값 생성
    ...
    img_target_cols = ["page_id", "brand_name", "image_sequence", "image_url", "has_alt", "alt_contents"]
    img_db_df = img_db_df[img_target_cols].drop_duplicates(
        subset=["page_id", "image_sequence"], keep="first"
    )

    # 4. DB 기존 중복 제외
    try:
        existing_img_query = "SELECT page_id, image_sequence FROM image_data_table"
        existing_img_df = pd.read_sql(existing_img_query, con=engine)
        if not existing_img_df.empty:
            existing_set = set(zip(existing_img_df["page_id"], existing_img_df["image_sequence"]))
            img_db_df = img_db_df[
                ~img_db_df.apply(lambda r: (r["page_id"], r["image_sequence"]) in existing_set, axis=1)
            ]
    except Exception:
        pass

    if img_db_df.empty:
        print("   ✅ 이미 적재가 완료되었거나 새로 넣을 이미지 데이터가 없습니다.")
        return

    # 5. DB INSERT 실행
    try:
        inserted_count = len(img_db_df)
        inserted_products = img_db_df["page_id"].nunique()
        ...
        print(f"   📊 [이미지 적재 상세 결과]")
        print(f"      - 수집된 원본 URL: {total_raw_rows:,}개 ({total_raw_products:,}개 상품)")
        print(f"      - page_id 매핑 성공: {matched_count:,}개 URL")
        print(f"      - 🚀 최종 DB 적재 성공: {inserted_products:,}개 상품 / {inserted_count:,}개 이미지 URL")
    except Exception as e:
        print(f"   ❌ [실패] image_data_table 적재 중 에러 발생: {e}")


def update_existing_alt_data(image_dfs: list):
    """기존 image_data_table DB 데이터의 alt_contents 및 has_alt UPDATE"""
    if not image_dfs:
        return
    ...
    product_id_df["product_name"] = product_id_df["product_name"].astype(str).str.strip()
    combined_img["brand_name"] = combined_img["brand_name"].astype(str).str.strip()
    product_id_df["brand_name"] = product_id_df["brand_name"].astype(str).str.strip()
    ...
    combined_img["page_id"] = combined_img.apply(
        lambda r: page_map.get((r["brand_name"], r["product_name"])), axis=1
    )
    ...
    # 1. has_alt 처리
    if "has_alt" in combined_img.columns:
        combined_img["has_alt"] = combined_img["has_alt"].astype(str).str.upper().isin(["TRUE", "1", "Y", "O", "유", "YES"])
    else:
        combined_img["has_alt"] = False

    # 2. alt_contents 정제
    if "alt_contents" in combined_img.columns:
...
def run_full_pipeline(target_directory: Path = CURRENT_DIR):
    print("=" * 65)
    print("🚀 전체 데이터 수집 및 DB 적재/업데이트 파이프라인 시작 (대기업 전용)")
    print("=" * 65)
    ...
    for tag, files in tagged_files.items():
        ...
        if has_product:
            p_df = process_product_csv(files["product"], tag_prefix=tag)

            if "text" in files:
                t_df = process_text_csv(files["text"], tag_prefix=tag)
            ...
            if "vl" in files:
                vl_df = process_vl_csv(files["vl"], tag_prefix=tag)
            ...
            if "image" in files:
                insert_image_dfs.append(process_image_csv(files["image"], tag_prefix=tag))

        else:
            if "text" in files:
                standalone_text_dfs.append(process_text_csv(files["text"], tag_prefix=tag))
            if "vl" in files:
                standalone_vl_dfs.append(process_vl_csv(files["vl"], tag_prefix=tag))
            
            if "image" in files:
                update_image_dfs.append(process_image_csv(files["image"], tag_prefix=tag))

    # PHASE 2-1: 기존 DB UPDATE (본문, VL)
    ...
    if new_product_dfs:
        ...
        # [수정 완료] brand_name과 product_name 복합 키 기준 중복 제거
        p_df = p_df.drop_duplicates(subset=["brand_name", "product_name"], keep="first")
    ...
    # PHASE 3: image_data_table 적재 (INSERT) 및 수정 (UPDATE)
    if insert_image_dfs:
