...
# -------------------------------------------------------------------
# 1. 파일별 전처리 헬퍼 함수 (대기업 맞춤형)
# -------------------------------------------------------------------
def process_product_csv(file_path: Path, tag_prefix: str = "") -> pd.DataFrame:
    """Product CSV 처리 (cat 컬럼 -> product_cat으로 매핑 | 브랜드: CJ 고정 | 대기업)"""
    ...
    print(f"   └ 완료 ({len(result_df):,}행 | 브랜드: CJ)")
    return result_df


def process_text_csv(file_path: Path, tag_prefix: str = "") -> pd.DataFrame:
    """Text CSV 처리 (DB 매칭용 브랜드 CJ 고정, text_contents만 추출)"""
...
def process_image_csv(file_path: Path, tag_prefix: str = "") -> pd.DataFrame:
    """Image CSV 파일 처리 (alt속성 유무와 관계없이 유연하게 파싱)"""
...
def process_jl_jsonl(file_path: Path) -> dict:
    """JSONL 파일 처리 (대기업 타겟 키: jsonld_blocks)"""
