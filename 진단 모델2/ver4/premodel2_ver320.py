...
class KeywordStuffingResult:
    ...
    raw_score: float
    is_stuffing: bool
    title_raw_score: float  # 7:3 나누기 전 상품명 스터핑 원점수
    body_raw_score: float   # 7:3 나누기 전 본문 스터핑 원점수
    noun_penalty: float
...
class GEOTotalEvaluationResult:
    ...
    def print_summary(self):
        ...
        print(f"    ├─ 원점수: 상품명({ks.title_raw_score}) | 본문({ks.body_raw_score})")
...
class GEOScorer:
    ...
    # =========================================================================
    # ③-1. 상품명 전용 키워드 스터핑 평가 (보조 메서드)
    # =========================================================================
    def calculate_title_stuffing(self, title_text: str) -> float:
        clean_title = self._clean_text(title_text) if title_text else ""
        if not clean_title:
            return 0.0

        pos_tags = self._extract_pos(clean_title)
        if not pos_tags:
            return 0.0

        nouns = [word for word, tag in pos_tags if tag.startswith('N')]
        
        # 1. 동일 명사 반복 감점 (3회 이상 반복 시 0.4, 2회 0.2)
        noun_counts = Counter(nouns)
        max_duplicate = max(noun_counts.values()) if noun_counts else 0
        dup_penalty = 0.4 if max_duplicate >= 3 else (0.2 if max_duplicate == 2 else 0.0)

        # 2. 명사 개수 과다(단어 나열) 감점 (8개 이상 0.35, 6개 이상 0.15)
        len_penalty = 0.35 if len(nouns) >= 8 else (0.15 if len(nouns) >= 6 else 0.0)

        # 3. 구분자/특수문자 남발 감점
        delimiter_count = len(re.findall(r'[/|\[\](),_]', clean_title))
        delim_penalty = 0.25 if delimiter_count >= 4 else (0.1 if delimiter_count >= 2 else 0.0)

        title_raw_score = max(0.0, 1.0 - (dup_penalty + len_penalty + delim_penalty))
        return round(title_raw_score, 3)

    # =========================================================================
    # ③-2. 키워드 스터핑 최종 평가 (7:3 가중치 적용 및 원점수 기록)
    # =========================================================================
    def calculate_keyword_stuffing(self, title_text: str = "", page_text: str = "") -> KeywordStuffingResult:
        # 1. 상품명 원점수 계산 (비중 0.7)
        title_raw_score = self.calculate_title_stuffing(title_text)

        # 2. 본문 원점수 계산 (비중 0.3)
        clean_page = self._clean_text(page_text) if page_text else ""
        
        if not clean_page:
            body_raw_score = 0.0
            noun_penalty = grammatical_penalty = pattern_penalty = 0.0
            noun_ratio = grammatical_ratio = 0.0
            pattern_count = 0
        else:
            ...
            if not pos_tags:
                ...
                body_raw_score = max(0.0, 1.0 - (noun_penalty + grammatical_penalty + pattern_penalty))

        # 3. 7:3 비율 최종 점수 산출
        final_score = round((title_raw_score * 0.7) + (body_raw_score * 0.3), 3)
        is_stuffing = final_score < 0.6  # 모니터링 기준

        return KeywordStuffingResult(
            final_score=final_score,
            raw_score=final_score,
            is_stuffing=is_stuffing,
            title_raw_score=round(title_raw_score, 3),  # 상품명 원점수
            body_raw_score=round(body_raw_score, 3),    # 본문 원점수
    ...
    def evaluate_single_alt(self, alt_text: str) -> SingleAltEvaluationResult:
        ...
        # 보정: alt 속성이 없거나 빈 값이면(has_alt가 False면) 즉시 0점 반환
        if not clean_alt:
        ...
        # 기존 패턴 + 하이픈/언더바 뒤 영문+숫자 혼합 무작위 코드 감지 패턴 추가
        invalid_patterns = (
            r'(\.(jpg|jpeg|png|gif|webp|bmp)$)|'       # 확장자
            r'(img[_-]?\d+|image[_-]?\d+|picture[_-]?\d+)|'  # img_01, image-01 등
            r'([_-][a-z0-9]{4,10}\b)'                   # -sl3iq, _a1b2c3 등 무작위 난수 코드
        )
    ...
    def calculate_avg_alt_score(self, image_list: List[Dict[str, Any]]) -> ImageAltEvaluationResult:
        # 보정: 이미지가 아예 없으면 1.0(만점)이 아니라 0점 반환
        if not image_list:
    ...
    def evaluate_page(
        ...
        # 🚨 [수정 완료] product_name(title_text)과 page_text 인자 2개를 정확히 전달합니다.
        s3 = self.calculate_keyword_stuffing(
