...
class GEOScorer:
    ...
    def calculate_json_ld_score(self, json_ld_str: str) -> JsonLdEvaluationResult:
        ...
        # json_ld_str이 이미 리스트/딕셔너리 객체일 경우와 문자열일 경우 모두 대응
        try:
            if isinstance(json_ld_str, str):
                if not json_ld_str.strip():
                    return default_fail
                
                data = json.loads(json_ld_str)

                # 👇 [이 3줄을 추가해 주세요!]
                # 최상위가 List([ {...} ]) 형태일 경우, 내부의 첫 번째 dict 요소 추출
        ...
        # [수정 포인트] 최상위가 List([ ... ]) 형태인 경우 첫 번째 요소 추출
        if isinstance(data, list) and len(data) > 0:
            data = data[0]
        ...
        if has_product and has_graph:
            ...
            parsing_score = 0.8  # Product 단독 존재 시 점수 완화 (기존 0.7 -> 0.8)
        elif has_graph:
            parsing_score = 0.3
        else:
            return default_fail
